...
@section('content')
    <div class="max-w-md p-6 mx-auto bg-[#e8e8e8]">
        <!-- Header Section -->
        <div class="mb-8">
            <h2 class="text-2xl font-bold text-gray-800">Edit Kategori Baru</h2>
            <p class="text-sm text-gray-500 mt-1">Isi form berikut untuk memperbarui kategori baru</p>
        </div>

        <!-- Form Section -->
        <form action="{{ route('categories.update', $category) }}" method="POST" class="bg-white rounded-md border border-gray-200 shadow-sm p-6">
            @csrf
            @method('PUT')
